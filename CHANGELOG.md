...
### Added
- **Voice — Phase 2b (user-created personas):** New voice audition flow integrated into `/personnas/new`. Genre and age are auto-detected from the first name (~500 French names dictionary) and from the prompt content (regex on "X ans", gendered keyword scan). The audition shows 5 candidate ElevenLabs voices ranked by attribute match score, plays the candidate's CDN `preview_url` instantly (zero-cost), and persists the chosen voice on agent creation. The persona's own greeting preview ("Bonjour, je m'appelle <name>") is generated server-side in the background via Next.js `after()` so the create response is not blocked. New backend: `src/lib/voice/catalog.ts` (cached ElevenLabs catalog), `matcher.ts` (scoring), `personaInference.ts` (heuristics), `/api/voice/recommend`. Extended: `/api/voice/tts` (accepts direct `voiceId`), `/api/agents` POST (stores `voice_profile`, triggers preview generation). New components: `VoiceAudition`, extended `VoicePlayer`. Inline hint surfaces missing required fields next to the disabled Create button.
- **Voice — Phase 2 (Théo and Oriane):** Add per-persona voices via `voice_profile` SQL updates and the new generic `scripts/generate-persona-preview.mjs`. Théo uses Hugo (Warm and Grounded); Oriane uses Adina. Discovery: Oriane's showcase had been silently redefined as "Chargée de mission RH, 34 ans" in the April home rebuild while her system prompt remained Alexis's original "étudiante M1 sciences sociales, 22 ans". The showcase has been reverted to match the prompt; the inconsistency is now documented.
- **Voice — Phase 1 (Jade only):** Per-persona voice configuration via new `agents.voice_profile` jsonb column (migration `20260616_add_agents_voice_profile.sql`). Voice editorially chosen by admin and fixed per persona; the student never picks the voice. Architecture is provider-extensible but committed to ElevenLabs in Phase 1.
- **Voice TTS Route:** Add `POST /api/voice/tts` (server-side ElevenLabs call) with mandatory cache in Supabase Storage bucket `voice-cache` (sha256(voiceId + text)), 600-char limit, and 503 fallback when the provider is misconfigured. ELEVENLABS_API_KEY is server-only and never exposed.
- **VoicePlayer Component:** Add reusable `<VoicePlayer />` with `preview` mode (static URL) and `tts` mode (on-demand generation via the route), play/pause/replay controls, loading and error states, and optional `autoplay` prop.
- **Voice Preview on Cards:** Wire `<VoicePlayer mode="preview" />` on the home showcase card and on the `/personnas` agent card for Jade. Oriane and Théo display a "Voix bientôt" badge until their `voice_profile` is configured.
- **Voice in Interview Chat:** Add a play button below each assistant message in `ChatMessage` (only shown when the persona has a configured voice and the message is not streaming). Add a "Lecture automatique" toggle in the `/interview` header, persisted to localStorage, that triggers autoplay only on the latest non-streaming assistant message.
- **Voice Audition & Preview Scripts:** Add `scripts/audition-jade-voices.mjs` (generates 3 candidate MP3s saying Jade's greeting for editorial choice) and `scripts/generate-jade-preview.mjs` (regenerates Jade's official preview and uploads to the `voice-cache` bucket).
- **Voice Docs:** Add `docs/voice/VOICE_PHASE_1_PLAN.md` (full spec, sub-steps, ownership matrix, risks), `docs/voice/ELEVENLABS_API_KEY_REFERENCE.md` (full catalog of ElevenLabs endpoints with usage, risk, and "what to enable if you want to do X"), and `docs/voice/JADE_VOICE_DECISION.md` (Jade's voice selection process and history — currently Victoria — Content Creator).
